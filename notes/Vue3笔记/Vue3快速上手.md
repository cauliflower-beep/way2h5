...
>    }    
>    function changeCar() {
>      car = {brand:'byd',price = 50} // 这么写页面是不会改变的
>        car = reactive({brand:'byd',price = 50}) // 这么写页面也是不会改变的，相当于重新创建了一个响应式对象
>        Object.assign(car,{brand:'byd',price:30})
>    }
>    function test(){
>      obj.value.a.b.c.d = 999
>    }
...
<template>
  <div class="person">
    姓：<input type="text" v-model="firstName"> <br> // v-model双向绑定 v-model:value
...
- 作用：监视数据的变化（和`Vue2`中的`watch`作用一致，写法不一致）
